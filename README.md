...
|---|---|---|
| **LiveLearn (`lile`) integration** | Studio talks to the **lile** online-learning daemon (lives in [heiervang-technologies/agi](https://github.com/heiervang-technologies/agi) since 2026-05-15). The `/lile` page handles capsule lifecycle, live loss / KL / queue-depth charts, snapshots + trajectory tabs, and feedback. Daemon runs in four modes (`external`, `spawned`, `external-unreachable`, `unconfigured`) — Studio detects which at request time via `lile_available()`. | [Studio routes](studio/backend/routes/lile.py) · [Contract](docs/studio-lile-integration.md) · [Frontend](studio/frontend/src/features/lile) · [agi repo](https://github.com/heiervang-technologies/agi) |
| **Prompt Baking** | 4th training method alongside QLoRA / LoRA / Full fine-tune. Distils a system prompt into LoRA weights via KL divergence so the model exhibits the prompted behavior at zero inference-time cost. Integrates [marksverdhei/bakery](https://github.com/marksverdhei/bakery). | [HT-CHANGELOG](HT-CHANGELOG.md) |
| **Gemma 4 12B Unified** | Wires Google's `gemma-4-12B-it` (encoder-free Any-to-Any, 11.95B params, 256K ctx) across the mapper + 5 Studio surfaces ahead of upstream. `transformers` cap raised to `<5.12` so the `gemma4_unified` arch loads; existing `gemma4` vision/PEFT dispatch covers both text-only and multimodal training. | [Smoke test](tests/test_gemma4_12b_smoke.py) · [HT-CHANGELOG](HT-CHANGELOG.md) |
...
| **Detached Attention** | New `detach_attention=True` flag in `get_peft_model` to skip autograd for attention layers when using MLP-only LoRA, saving ~25% VRAM and boosting throughput. | [PR #64](https://github.com/heiervang-technologies/ht-unsloth/pull/64) |
...
| **Fork infrastructure** | In-repo `.venv` for editable installs, HT branding in Studio, fork-sync CI, HT Discussions links, [rebase resumption guide](docs/rebase-resumption-guide.md) for upstream catch-ups. | [HT-CHANGELOG](HT-CHANGELOG.md) |
