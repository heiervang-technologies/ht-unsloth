...
# Train in the same process — returns step_token N
curl -sS http://127.0.0.1:8768/v1/train \
  -H 'content-type: application/json' \
  -d '{"objective":"sft","samples":[{"prompt":"2+2?","response":"4."}]}' | jq

# Next chat that MUST see batch N: pass after_step_token
curl -sS http://127.0.0.1:8768/v1/chat/completions \
  -H 'content-type: application/json' \
  -d '{"messages":[{"role":"user","content":"2+2?"}],"after_step_token":N}' | jq
```

The response body's `lile.step_cursor` is guaranteed ≥ `after_step_token`. That's the "post a batch, next inference sees it" promise as a contract.
...
| Route | Purpose |
|---|---|
| `POST /v1/chat/completions` | OpenAI-compatible chat (streaming + non-streaming). Accepts `after_step_token` to block on a training commit. Returns `lile.{response_id, step_cursor, latency_s}`. |
| `POST /v1/train` | Enqueue a training batch. Request body: `{objective, samples[], ...objective-specific params}`. Returns `step_token`. |
| `POST /v1/feedback` | Route feedback (`binary`, `rewrite`, `preferred`, `nl_critique`) on a prior `response_id` to the matching objective (KTO / weighted SFT / hinge / CoH or CCPD v2). Returns `step_token`. |
| `POST /v1/wait` | Block until a given step_token is applied. |
...
| `GET  /health` | `{ok, model, queue_depth, step_cursor, merges}`. |
...
├── commit_stream.py       # /v1/steps/stream SSE primitive
...
1. **Step cursor is monotone and contract-typed.** `tests/test_queue_cursor.py`, `tests/test_merge_and_e2e.py::test_controller_step_cursor_e2e`.
